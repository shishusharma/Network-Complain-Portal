...
if (strlen($_SESSION['tid']==0)) {
  ...
  // echo $brands;
    ...
    //  $query_run = mysqli_query($con, $query);
      $techid1=$_SESSION['tid'];
      $query1=mysqli_query($con,"select tid from usercomplain where tid='$techid1' and status='0'");
     $totaltech=mysqli_num_rows($query1);
     $query2=mysqli_query($con,"select tid from usercomplain where tid='$techid1' and status='1'");
    $totaltech1=mysqli_num_rows($query2);
    if(($totaltech1-$totaltech)==$totaltech1){
      $query3 = "update tech set tstatus='0' where tid='$techid1'";
      $query_run = mysqli_query($con, $query3);
    }else{
      $query3 = "update tech set tstatus='1' where tid='$techid1'";
      $query_run = mysqli_query($con, $query3);
    }
  ...
  <div class="card-body">
     ...
     <th>Action</th>
          ...
          <tbody>
                    <?php
                      $tid=$_SESSION['tid'];
                    $ret=mysqli_query($con,"SELECT * from usercomplain INNER JOIN userlogin ON usercomplain.uid=userlogin.uid and usercomplain.status=0 and usercomplain.tid=$tid ;");
