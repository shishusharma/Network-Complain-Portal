      ...
      foreach($brands as $bds1)
       {
          ...
          $q1="select tname from tech where tid='$techid'";
           ...
           $query_run = mysqli_query($con, $query);
             ...
             $query1=mysqli_query($con,"select tid from usercomplain where tid='$techid' and status=0");
            $totaltech=mysqli_num_rows($query1);
            $query2=mysqli_query($con,"select tid from usercomplain where tid='$techid' and status=1");
            $totaltech1=mysqli_num_rows($query2);
            if(($totaltech-$totaltech1)==0){
              $query3 = "update tech set tstatus='0' where tid='$techid'";
              $query_run = mysqli_query($con, $query3);
            }else{
              $query3 = "update tech set tstatus='1' where tid='$techid'";
              $query_run = mysqli_query($con, $query3);
            }
                              ...
                              <th>Choose A Technician</th>
                                 <td>
                                   <select class="form-select" name="techname" aria-label="Default select example">
                                    ...
                                    <optgroup label="Select Technician">

                                    <!-- <option value="11001101">kumar shingh</option>
                                    <option value="11001102">kancha</option>
                                    <option value="11001103">lala</option> -->
                                    <?php
                                      $techq="SELECT * From tech";
                                      $result=mysqli_query($con,$techq);
                                      if(mysqli_num_rows($result)>0){
                                        while($tech=mysqli_fetch_assoc($result)){
                                          ?>
                                          <option value="<?php echo $tech['tid'] ?>">
                                          <?php echo $tech['tname'] ?>
                                        </option>
